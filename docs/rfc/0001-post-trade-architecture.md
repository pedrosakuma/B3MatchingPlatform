...
| ADR 0002  | Per-trade audit log shape                              | Blocks #329 implementation; unblocks ADR 0008. **Must explicitly decide** whether the audit infrastructure is fills-only or extensible to order-lifecycle events (add/modify/cancel/phase); ADR 0006 depends on this choice — see note below the table. | Planned, **blocks #329**.       |
