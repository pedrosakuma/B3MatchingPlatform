...
| <a id="gap-19"></a>GAP-19 | 4.8 | **Mass Cancel (`OrderMassActionRequest` 701 / `OrderMassActionReport` 702)** with filters Side / SecurityID / OrdTagID / Asset, plus mass-cancel-on-behalf. | partial | Engine path is complete: `MassCancelCommand` carries all five filters and `MatchingEngine.MatchesMassCancelFilter` (`src/B3.Exchange.Matching/MatchingEngine.cs:1072-1081`) applies them; the gateway rejects unsupported `MassActionType` values with spec `MassActionRejectReason`, not free-text. **However `InboundMessageDecoder.SimpleNewOrder` does not decode `ordTagID` (id 35505) or `investorID` (id 35508) from `SimpleNewOrder` (template 100), so resting orders entered via the simple path silently miss `OrdTagID` / `InvestorID` mass-cancel filters.** `NewOrderSingle` (102) is wired; `SimpleNewOrder` wire-up is the residual gap. | medium | [#412](https://github.com/pedrosakuma/B3MatchingPlatform/issues/412) (closed), narrowed by [#449](https://github.com/pedrosakuma/B3MatchingPlatform/issues/449) |
