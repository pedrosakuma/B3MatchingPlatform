...
| <a id="gap-23"></a>GAP-23 | 7.1.7–7.1.14 | `GTC` / `GTD` / `MOC` (AtClose) / `MOA` (GoodForAuction) time-in-force | done | — | All four TIFs are classified at the decoder and gated by trading phase in `MatchingEngine.NewOrder`. GTD/MOC/MOA only accept in the matching auction (Reserved) / final closing call respectively. |
